# qidi_pa_measure.py - Stage 3: measure pressure advance per envelope point
#
...
#
#   Results go to ~/printer_data/qidi_pa/pa_table.json
...
class QidiPAMeasure:
    def __init__(self, config):
        ...
        # MUST be configurable, and must match [qidi_flow_ramp]'s value. This
        # used to be the module constant, so a machine whose cell read anything
        # other than 182.96 counts/gf got an abort at the wrong FORCE while the
        # console cheerfully printed "1650 gf". At 120 counts/gf that is 2516 gf
        # on a cell rated for 2000 - the one protection this printer has, set
        # 26% above the breaking point. Fixed 2026-09-17.
        ...
        self.gcode.register_command('QIDI_PA_MEASURE', self.cmd_MEASURE,
    ...
    def _record(self, payload):
        try:
            ...
            logging.exception("qidi_pa_measure: could not write report")
            return False

    def _parts(self, gcmd):
        root = self.printer.lookup_object(self.root_name, None)
        if root is None:
            raise gcmd.error("pa: no object %s" % (self.root_name,))
        ...
        if raw is None or oid is None or not callable(read):
            raise gcmd.error("pa: sensor not usable - run QIDI_CS_LOCATE")
    ...
    def _envelope(self, gcmd):
        ...
        try:
            ...
            raise gcmd.error("pa: no usable envelope.json - run "
    ...
    def _collect(self, gcmd, raw, oid, mcu, until_pt, tare, rtt_cap):
        ...
        while True:
            ...
            if abs(rel) > abort_counts:
                raise gcmd.error("pa: FORCE ABORT %.0f gf exceeds %.0f gf"
    ...
    cmd_MEASURE_help = ("Stage 3 - measure pressure advance at each envelope "
                        "point by fitting the melt time constant. HEATS AND "
    ...
    def cmd_MEASURE(self, gcmd):
        ...
        try:
            ...
            logging.exception("qidi_pa_measure: unhandled error")
            raise gcmd.error("pa: internal error, printer left running: "
        ...
        if flow is not None and accel is not None:
            ...
            if want:
                pts = [p for p in pts if p['name'].replace(' ', '')
                       ...
                       .replace(',', '')]
                if not pts:
                    raise gcmd.error("pa: no envelope point matching %r"
                                     % (want,))
        for p in pts:
    ...
    def _run(self, gcmd):
        ...
        gcmd.respond_info(
            "pa: Stage 3 - fitting the melt time constant directly. Klipper "
        ...
        if dry:
            gcmd.respond_info("pa: DRY - nothing heated, nothing moved.")
            return
        ...
        if 'xyz' not in (toolhead.get_status(self.reactor.monotonic())
                         .get('homed_axes', '')):
            raise gcmd.error("pa: home the printer first (G28)")
        ...
        try:
            ...
            try:
                ...
                logging.exception("qidi_pa_measure: cleanup motion limits")
            try:
                self._script("M83")
                self._script("G1 E-1.0 F1800")
                self._wipe()
            except Exception:
                logging.exception("qidi_pa_measure: cleanup retract/wipe")
            if started:
                try:
                    self._script("M106 P%d S0" % (FILTER_M106_P,))
                except Exception:
                    logging.exception("qidi_pa_measure: cleanup filtration")
    ...
    def _measure_point(self, gcmd, p, leg_s, cycles, blocks, read, raw, oid,
        ...
        gcmd.respond_info("pa: %s - Q %.2f mm3/s, v_E %.3f (%.3f..%.3f), "
                          "a_E %.1f, dv %.0f%%%s"
    ...
    def _summary(self, gcmd, results, geom, leg_s, cycles, blocks,
        ...
        gcmd.respond_info("pa: written to %s" % (self.report_path,))


def load_config(config):
    return QidiPAMeasure(config)
