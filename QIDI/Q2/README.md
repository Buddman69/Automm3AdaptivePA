...
## Status: ported from the X-Max 4, not yet verified on a Q2

This folder is a straight duplicate of the X-Max 4 build, kept as its own
complete codebase rather than sharing files with it — see the root README for
why. **It has not been run end-to-end on a Q2.** What is and is not
established:

**Verified** (checked against the Q2's own firmware and config): the Q2 runs
**byte-identical toolhead firmware** to the X-Max 4, the same `[probe_air]`
wiring (`sensor_type: c_sensor` on `THR:PB3` / `THR:PB4`), the same extruder
gearing, and the same disabled Klipper extrusion guards. That is why this
starts from the X-Max 4 code rather than from nothing — the sensor chain and
the safety gap underneath it are the same machine at that level.

**Not verified — assumed, pending real measurement on a Q2:**

- **`counts_per_gf`**, defaulted here to the X-Max 4's measured 182.96. This is
  a property of one physical cell and its mount, not of the model. Do not
  trust it — measure your own, exactly as the README below describes, before
  this means anything on force.
- **The 1650 gf abort ceiling**, derived from the X-Max 4's own calibration run
  (2175 gf with no hysteresis). Re-derive it once you have your own
  counts/gf.
- **Every flow, wipe-travel and timing constant** in `qidi_flow_ramp.py` and
  `qidi_pa_measure.py` — tuned on the X-Max 4's melt chamber, TMC current and
  wiper geometry.

**Different, already changed:** the Q2 ships a **0.4 mm nozzle** against the
X-Max 4's 0.6 mm, so the wizard's nozzle-screen default has been set
accordingly here (`qidi_cal_wizard.py`). Layer/width defaults follow from that.

**Known to be moving:** Budd is developing altered firmware for the Q2. This
folder will change once that lands — it is a starting point, not a finished
port.

Until then, treat this exactly as a fresh bring-up on new hardware: **Step 1
of the README below (`QIDI_CS_READ SECONDS=10`, confirming the sensor moves
under a hand press) is not optional here.** It has not been done on this
machine yet.
...
change, and committing it would bloat the history. The source it is built from
is all here.)
