# qidi_flow_bed_search.py - Stage 1, run over the BED instead of the chute
#
...
#
#
# A COPY of qidi_flow_ramp.py, kept as a separate file on purpose - see
# CHANGELOG/ and qidi_auto_cal_bed.py's header for why. The measurement logic
# below (settling, abort conditions, the search algorithm, the working-max
# maths) is UNCHANGED from the chute version. What changed is WHERE it runs:
#
#   * extrusion and taring happen at bed centre, not the chute
#   * the wiper is still only reachable at the chute, so every wipe - the
#     internal ones during the search AND the standalone command - now means
#     travelling there and back, not wiping in place. See _wipe_at_chute().
#   * bed centre is a LIVE position, captured by QIDI_BED_PREPARE right after
#     a mandatory G28, not a fixed macro constant like the chute's park_x.
#     Every move there is guarded: if the toolhead's actual Z does not match
#     what was recorded, this refuses rather than risk driving the nozzle
#     into a bed that turned out not to still be lowered. See
#     _move_to_bed_centre().
#
...
#   because the baseline wanders several gf over minutes. Here that tare is
#   taken AT BED CENTRE, not at the chute - matching every other stage in this
#   project, which always tares where it is about to measure rather than
#   somewhere else and hoping it transfers.
#
...
#
#   QIDI_FLOW_BED_SEARCH TEMP=275     needs QIDI_BED_PREPARE run first (or
#                                     QIDI_AUTO_CALIBRATE_BED, which does it)
#   QIDI_FLOW_BED_WIPE                wipe at the chute, return to bed centre
#
#   Results are appended to ~/printer_data/qidi_pa/flow_ramp.json - the SAME
#   file the chute version writes. Deliberate, not an oversight: QIDI_PA_TABLE
#   and QIDI_PA_ENVELOPE are reused unmodified and read from a fixed path -
#   see qidi_auto_cal_bed.py's header for the full reasoning and the residual
#   risk that accepts.
...
# The TOP-LEVEL between-stage wipe (QIDI_FLOW_BED_WIPE run on its own, or
# called between stages by QIDI_AUTO_CALIBRATE_BED) is deliberately lighter
# than the internal round-trip wipes above: 2 long strokes, no short strokes -
# Budd's own spec for the wipe/return steps between stages. The internal
# wipes, triggered every SEARCH_WIPE_VOLUME_MM3 during the search itself,
# keep the FULL WIPE_PASSES/WIPE_SHORT_PASSES pattern above, unchanged from
# the chute routine - that quality of clean is what stops a blob corrupting
# the rest of the search, and this addition does not touch it.
BETWEEN_STAGE_WIPE_PASSES = 2
BETWEEN_STAGE_WIPE_SHORT_PASSES = 0
...
# Feedrate for the X/Y travel between the chute and bed centre. Not the wipe
# feedrate (WIPE_FEED_FAST/SLOW, untouched) - this is plain travel.
...
# ---- adaptive search (QIDI_FLOW_BED_SEARCH) ---------------------------------
...
class QidiFlowBedSearch:
    def __init__(self, config):
        ...
        # QIDI_PA_ENVELOPE can find it. See this file's header.
        self.report_path = os.path.join(self.out_dir, 'flow_ramp.json')
        self.position_path = os.path.join(self.out_dir, 'bed_position.json')
        self.gcode.register_command('QIDI_FLOW_BED_RAMP', self.cmd_RAMP,
                                    desc=self.cmd_RAMP_help)
        self.gcode.register_command('QIDI_FLOW_BED_WIPE', self.cmd_WIPE,
                                    desc=self.cmd_WIPE_help)
        self.gcode.register_command('QIDI_FLOW_BED_SEARCH', self.cmd_SEARCH,
    ...
    def _guard(self, gcmd, fn):
        ...
        try:
            ...
            logging.exception("qidi_flow_bed_search: unhandled error")
            raise gcmd.error("flow_bed_search: internal error, printer left "
                             "running: %s: %s"
    ...
    def _read_bed_position(self, gcmd):
        try:
            ...
            raise gcmd.error(
                "flow_bed_search: no bed_position.json - run "
                "QIDI_BED_PREPARE first (or QIDI_AUTO_CALIBRATE_BED, which "
                "does it for you)")

    def _move_to_bed_centre(self, gcmd, toolhead):
        bx, by, bed_z = self._read_bed_position(gcmd)
        cur = toolhead.get_position()
        # The bed must actually BE at the recorded Z, or moving to this X/Y
        # at whatever Z currently is could put the nozzle into the bed - e.g.
        # a real print ran and re-homed since QIDI_BED_PREPARE last wrote
        # this file. This is the one new safety check this addition needed
        # that nothing existing already provided.
        if abs(cur[2] - bed_z) > BED_Z_TOLERANCE_MM:
            raise gcmd.error(
                "flow_bed_search: expected the bed at Z%.1f (from the last "
    ...
    def _record(self, payload):
        ...
        try:
            ...
            logging.exception("qidi_flow_bed_search: could not write report")
            return False

    def _sensor(self, gcmd):
        root = self.printer.lookup_object(self.root_name, None)
        if root is None:
            raise gcmd.error("flow_bed_search: no object %s" % (self.root_name,))
        s = getattr(root, self.sensor_attr, None)
        if s is None:
            raise gcmd.error("flow_bed_search: %s has no %s - run QIDI_CS_LOCATE"
                             % (self.root_name, self.sensor_attr))
        read = getattr(s, 'read_origin_data', None)
        if not callable(read):
            raise gcmd.error("flow_bed_search: read_origin_data is not callable")
        return read
    ...
    def _sample(self, read, n, interval, gcmd):
        ...
        for _ in range(n):
            try:
                ...
                raise gcmd.error("flow_bed_search: force read failed: %s" % (e,))
            if isinstance(v, (int, float)) and not isinstance(v, bool):
                vals.append(float(v))
            eventtime = self.reactor.pause(eventtime + interval)
        if not vals:
            raise gcmd.error("flow_bed_search: no numeric force readings")
        return vals
    ...
    def _wait_temp_stable(self, gcmd, target, tol, window, timeout):
        ...
        gcmd.respond_info("flow_bed_search: heating to %.0fC" % (target,))
        pheaters.set_temperature(heater, target, True)
        gcmd.respond_info("flow_bed_search: at target, waiting for a stable reading")
        ...
        while self.reactor.monotonic() < deadline:
            ...
            if len(hist) >= 4 and max(hist) - min(hist) <= tol \
                ...
                gcmd.respond_info("flow_bed_search: stable at %.1fC (spread %.2f over "
                                  "%.0fs)" % (_mean(hist), max(hist) - min(hist),
                                              window))
                return _mean(hist)
            eventtime = self.reactor.pause(eventtime + 0.5)
        raise gcmd.error("flow_bed_search: hotend did not settle within %.0fs"
                         % (timeout,))

    # --------------------------------------------------------------- the ramp

    cmd_RAMP_help = ("Flow ramp: measure melt force vs volumetric flow over "
                     "BED CENTRE - needs QIDI_BED_PREPARE run first. "
                     "[STEPS=8] [QMIN=2] [QMAX=9.98] [TEMP=275] "
    ...
    def _run_RAMP(self, gcmd):
        ...
        if qmax <= qmin:
            raise gcmd.error("flow_bed_search: QMAX must exceed QMIN")
        ...
        if worst_mm > max_e:
            raise gcmd.error(
                "flow_bed_search: worst case needs %.0f mm of filament, over the "
                "%.0f mm cap. Raise MAX_E or lower STEPS/REPEATS."
                % (worst_mm, max_e))

        gcmd.respond_info(
            "flow_bed_search: %d flows %.2f-%.2f mm3/s, %d measurements (%s), held "
        ...
        likely_mm = sum(q / A_FIL * min(SETTLE_MAX_S, max(SETTLE_MIN_S, 6.0))
                        for q, _l, _i in seq)
        gcmd.respond_info(
            "flow_bed_search: likely ~%.0f mm (%.2f g), worst case %.0f mm (%.2f g) "
            "if every point runs to the %.0fs cap"
            % (likely_mm, likely_mm * A_FIL * 1.09e-3,
               worst_mm, worst_mm * A_FIL * 1.09e-3, SETTLE_MAX_S))
        gcmd.respond_info("flow_bed_search: abort at %d counts above tare (%.0f gf, "
        ...
        if dry:
            ...
            for q, lab, i in seq:
                gcmd.respond_info("   %-7s step %2d  Q=%6.2f mm3/s  v_E=%5.2f"
                                  % (lab, i, q, q / A_FIL))
            gcmd.respond_info("flow_bed_search: DRY=1, nothing executed")
            return

        toolhead = self.printer.lookup_object('toolhead')
        if 'xyz' not in (toolhead.get_status(self.reactor.monotonic())
                         .get('homed_axes', '')):
            raise gcmd.error("flow_bed_search: home the printer first (G28)")
        ...
        try:
            ...
            gcmd.respond_info("flow_bed_search: moving to the purge chute "
                              "to heat")
            ...
            gcmd.respond_info("flow_bed_search: moving to bed centre")
            self._move_to_bed_centre(gcmd, toolhead)

            # Tare AT TEMPERATURE, AT BED CENTRE - the cold baseline does not
            # apply once the hotend is soaking the toolhead, and a baseline
            # taken somewhere else does not apply here either.
            tare_v = self._sample(read, 60, 1.0 / hz, gcmd)
            tare = _mean(tare_v)
            gcmd.respond_info("flow_bed_search: tare %.0f counts (stdev %.0f = %.2f gf)"
                              % (tare, _stdev(tare_v),
                        ...
                        if len(aborts) > max_aborts:
                            ...
                            abort_reason = (
                                ...
                                % (len(aborts), ", ".join("%.2f" % f for f
                                                          in sorted(aborts))))
                            gcmd.respond_info("flow_bed_search: %s" % (abort_reason,))
                            break
                if aborted:
                    break
        ...
        try:
            ...
            gcmd.respond_info("flow_bed_search: closing tare %.0f, drift over the run "
                              "%.2f gf" % (tare_end,
    ...
    def _settle_drift(self, gcmd, read, hz, tare, soak_max):
        ...
        while True:
            ...
            tare = _mean(v)
            waited += window
            gcmd.respond_info("flow_bed_search: drift %.1f gf/min after %.0fs soak"
                              % (rate, waited))
            if abs(rate) <= DRIFT_LIMIT_GF_MIN:
                return tare, rate
            if waited >= soak_max:
                raise gcmd.error(
                    "flow_bed_search: drift still %.1f gf/min after %.0fs of soaking. "
    ...
    def _filter(self, gcmd, on):
        try:
            ...
            gcmd.respond_info("flow_bed_search: air filtration %s"
                              % ("ON" if on else "off",))
        except Exception:
            logging.exception("qidi_flow_bed_search: filter control failed")
            gcmd.respond_info("flow_bed_search: WARNING could not switch the "
    ...
    def _wipe(self, gcmd, toolhead, lo, hi, passes, short_passes=None):
        ...
        self.gcode.run_script_from_command("\n".join(lines))
        toolhead.wait_moves()
        gcmd.respond_info("flow_bed_search: wiped X%.1f-%.1f x%d then "
                          "X%.1f-%.1f x%d, parked at X%.1f"
                          % (lo, hi, passes, lo, short_hi, short_passes, px))

    def _wipe_at_chute(self, gcmd, toolhead, lo, hi, passes,
                       short_passes=None, return_to_bed=True):
        ...
        is False only for the cooldown wipe, where the routine is ending and
        there is nothing to return FOR."""
        self.gcode.run_script_from_command("MOVE_TO_TRASH")
        toolhead.wait_moves()
        self._wipe(gcmd, toolhead, lo, hi, passes, short_passes)
        if return_to_bed:
            self._move_to_bed_centre(gcmd, toolhead)

    cmd_WIPE_help = ("Wipe at the chute, then return to bed centre. Lighter "
                     "than the internal round-trip wipes: 2 long strokes, no "
                     "short strokes, by default. "
                     "[PASSES=2] [SHORT_PASSES=0] [RETURN=1]")
    ...
    def _run_WIPE(self, gcmd):
        ...
        passes = gcmd.get_int('PASSES', BETWEEN_STAGE_WIPE_PASSES, minval=1,
                              maxval=20)
        short = gcmd.get_int('SHORT_PASSES', BETWEEN_STAGE_WIPE_SHORT_PASSES,
                             minval=0, maxval=20)
        ret = gcmd.get_int('RETURN', 1, minval=0, maxval=1)
        self._wipe_at_chute(gcmd, toolhead, lo, hi, passes, short,
                            return_to_bed=bool(ret))
    ...
    def _finish(self, gcmd, toolhead, retract, wipe, cooldown,
        ...
        try:
            ...
            if retract:
                ...
                toolhead.wait_moves()
                gcmd.respond_info("flow_bed_search: retracted %.2f mm" % (retract,))
        except Exception:
            logging.exception("qidi_flow_bed_search: retract failed")
            gcmd.respond_info("flow_bed_search: WARNING retract failed")

        if wipe:
            try:
                self._wipe_at_chute(gcmd, toolhead, wipe_lo, wipe_hi,
                                    wipe_passes)
            except Exception:
                logging.exception("qidi_flow_bed_search: wipe failed")
                gcmd.respond_info("flow_bed_search: WARNING wipe failed")

        if cooldown:
            try:
                heater = self.printer.lookup_object('extruder').get_heater()
                self.printer.lookup_object('heaters').set_temperature(heater, 0.)
                gcmd.respond_info("flow_bed_search: hotend off")
            except Exception:
                logging.exception("qidi_flow_bed_search: cooldown failed")
                gcmd.respond_info("flow_bed_search: WARNING could not turn "
                                  "the hotend off - do it manually")
    ...
    def _run_step(self, gcmd, read, toolhead, idx, q, hz, tare, abort_counts,
        ...
        gcmd.respond_info("flow_bed_search: %s Q=%.2f mm3/s  v_E=%.2f mm/s"
    ...
    def _leg_prep(self, gcmd, read, toolhead, lab, prime, hz, tare,
        ...
        used = 0.0
        gcmd.respond_info("flow_bed_search: --- %s: wipe and prime ---" % (lab,))
        if wipe:
            try:
                self.gcode.run_script_from_command("M83\nG1 E-1.000 F1800")
                toolhead.wait_moves()
                ...
                logging.exception("qidi_flow_bed_search: inter-leg wipe "
                                  "failed")
                gcmd.respond_info("flow_bed_search: WARNING inter-leg wipe "
                                  "failed")
        if prime:
            ...
            used += pr['e_mm']
            if pr.get('abort'):
                raise gcmd.error("flow_bed_search: aborted priming %s - %s"
                                 % (lab, pr['abort']))
        return used
    ...
    def _handle_abort(self, gcmd, toolhead, r, q, wipe=0, wipe_lo=None,
        ...
        if not recoverable:
            gcmd.respond_info("flow_bed_search: ABORT - %s" % (reason,))
            return True
        gcmd.respond_info("flow_bed_search: %.2f mm3/s did not equilibrate (%s) - "
        ...
        try:
            ...
            logging.exception("qidi_flow_bed_search: pressure relief failed")
        return False
    ...
    def _refine(self, gcmd, read, toolhead, results, refine, reps, hz, tare,
        ...
        if not ok_q:
            gcmd.respond_info("flow_bed_search: nothing equilibrated - no bracket to "
                              "refine")
            return used_mm
        lo = max(ok_q)
        ...
        if hi is None:
            gcmd.respond_info(
                "flow_bed_search: every flow tested equilibrated (highest %.2f mm3/s)"
        ...
        gcmd.respond_info(
            "flow_bed_search: --- refining between %.2f (equilibrated) and %.2f "
            "(did not) ---" % (lo, hi))
        gcmd.respond_info("flow_bed_search: %d points x%d: %s"
                          % (len(pts), reps,
        ...
        for lab, items in rlegs:
            try:
                ...
                gcmd.respond_info("flow_bed_search: %s prep failed - %s" % (lab, e))
                return used_mm
            for k, q in items:
                t_at = self.reactor.monotonic() - ramp_t0
    ...
    cmd_SEARCH_help = ("Find max volumetric flow by adaptive search, over BED "
                       "CENTRE instead of the chute - needs QIDI_BED_PREPARE "
                       "run first. Coarse stride, bisect, then +1 mm3/s. "
                       "[TEMP=275] [START=5] [STEP=10] [MAXQ=60] [DRY=0]")
    ...
    def _run_SEARCH(self, gcmd):
        ...
        while q <= maxq:
            ...
            q += step
        gcmd.respond_info(
            "flow_bed_search: adaptive search - coarse %s ... (stride %.0f, cap "
            "%.0f), then bisect, then +%.0f"
            % (", ".join("%.0f" % c for c in coarse[:5]), step, maxq, fine))
        gcmd.respond_info("flow_bed_search: each flow held up to %.0f-%.0fs "
        ...
        try:
            ...
            # ---- stage 1: coarse ----------------------------------------
            gcmd.respond_info("flow_bed_search: --- coarse ---")
            ...
            if first_fail is None:
                gcmd.respond_info(
                    "flow_bed_search: nothing failed up to %.0f mm3/s - the limit is "
                    "ABOVE the tested range. For a 0.6 nozzle that means the "
                    "measurement is wrong, not the filament." % (maxq,))
                limit = None
            elif last_pass <= 0.0:
                gcmd.respond_info(
                    "flow_bed_search: %.0f mm3/s already fails - the limit is BELOW "
                ...
                gcmd.respond_info("flow_bed_search: --- bisect %.1f (between %.0f "
                                  "pass and %.0f fail) ---"
                ...
                if not (m1 and m2):
                    gcmd.respond_info("flow_bed_search: bisect failed - stepping up "
                ...
                # ---- stage 3: fine -------------------------------------
                state['stage'] = 'fine'
                gcmd.respond_info("flow_bed_search: --- fine, +%.0f from %.1f ---"
                                  % (fine, base))
        ...
        gcmd.respond_info("flow_bed_search: ===================================")
        if limit:
            gcmd.respond_info("flow_bed_search: highest flow that equilibrated: "
                              "%.1f mm3/s" % (limit,))
            gcmd.respond_info("flow_bed_search: WORKING MAX = %.1f mm3/s  (%.1f x "
                              "%.2f = %.2f, rounded down to %.1f)"
                              % (wmax, limit, WORKING_MARGIN,
                                 limit * WORKING_MARGIN, WORKING_ROUND))
        else:
            gcmd.respond_info("flow_bed_search: no limit determined - see above")
        if marginal:
            gcmd.respond_info("flow_bed_search: NOTE the transition was marginal - a "
                              "flow failed then passed. Taking the lower, "
                              "conservative value.")
        fails = ", ".join("%.1f(%dx,%s)" % (q, n, state['why'].get(q, '?'))
                          for q, n in sorted(state['fails'].items()))
        gcmd.respond_info("flow_bed_search: failures: %s" % (fails or "none",))
        gcmd.respond_info("flow_bed_search: %d measurements, %.0f mm filament "
        ...
        gcmd.respond_info("flow_bed_search: written to %s"
    ...
    def _test_flow(self, gcmd, read, toolhead, q, hz, tare, abort_counts,
        ...
        if state['wipe_now']:
            try:
                self.gcode.run_script_from_command("M83\nG1 E-1.000 F1800")
                toolhead.wait_moves()
                self._wipe_at_chute(gcmd, toolhead, state['wipe_lo'],
                                    state['wipe_hi'], state['wipe_passes'])
            except Exception:
                logging.exception("qidi_flow_bed_search: search wipe failed")
        ...
        if ok:
            ...
            try:
                ...
                logging.exception("qidi_flow_bed_search: relief failed")
        state['passes'].setdefault(q, 0)
        if ok:
            state['passes'][q] += 1
        gcmd.respond_info("flow_bed_search:   %.1f mm3/s -> %s%s"
    ...
    def _report(self, gcmd, results, flows, tare, tare_end, aborted, reason,
        ...
        if tare is not None and tare_end is not None and ramp_len:
            ...
            gcmd.respond_info("flow_bed_search: correcting for %.1f gf of baseline "
        ...
        if skipped:
            gcmd.respond_info("flow_bed_search: excluded steps %s from the fit - not "
        ...
        payload_agg = agg
        if agg:
            gcmd.respond_info("flow_bed_search: per-flow summary")
        ...
        if len(agg) > 1:
            gcmd.respond_info("flow_bed_search: local exponent n = dlnF/dlnQ")
            locs = []
            ...
            for u, v in zip(locs, locs[1:]):
                if u['n'] <= 1.0 < v['n']:
                    ...
                    break
            if cross:
                gcmd.respond_info("flow_bed_search: n crosses 1.0 at %.2f mm3/s"
                                  % (cross,))
            base = [l['n'] for l in locs[:max(2, len(locs) // 3)]]
            if base:
                nb = _mean(base)
                gcmd.respond_info("flow_bed_search: low-flow plateau n = %.3f" % (nb,))
                c2 = None
                for u, v in zip(locs, locs[1:]):
                    if u['n'] <= 2 * nb < v['n']:
                        f = (2 * nb - u['n']) / (v['n'] - u['n'])
                        ...
                        break
                if c2:
                    gcmd.respond_info("flow_bed_search: n reaches 2x plateau (%.3f) "
                                      "at %.2f mm3/s" % (2 * nb, c2))
        else:
            payload_locs = []
        ...
        if fit:
            gcmd.respond_info("flow_bed_search: power law F = %.4g * Q^%.3f "
            ...
            gcmd.respond_info("flow_bed_search: not enough clean steps to fit")
        ...
        if limit:
            ...
            gcmd.respond_info("flow_bed_search: highest flow that equilibrated: "
                              "%.2f mm3/s" % (limit,))
            if first_bad:
                gcmd.respond_info("flow_bed_search: lowest that did not:          "
                                  "%.2f mm3/s" % (first_bad,))
                gcmd.respond_info("flow_bed_search: limit is bracketed to %.2f-%.2f "
                                  "(%.1f%% wide)"
                                  % (limit, first_bad,
                                     100.0 * (first_bad / limit - 1.0)))
            else:
                gcmd.respond_info("flow_bed_search: nothing failed - the limit is "
                                  "ABOVE the tested range, raise QMAX")
            gcmd.respond_info("flow_bed_search: WORKING MAX = %.2f mm3/s  "
                              "(%.2f x %.2f)" % (wmax, limit, WORKING_MARGIN))
            gcmd.respond_info("flow_bed_search: ===================================")
        else:
            gcmd.respond_info("flow_bed_search: no flow reached equilibrium - no "
...
def load_config(config):
    return QidiFlowBedSearch(config)
