...
def main():
    ...
    ok &= check("moves straight to bed centre - no MOVE_TO_TRASH "
    ...
    print("\n== the limits are captured once, before any motion ==")
    # Same concern as qidi_pa_measure.py's own test - see its comment. This
    # module's move (to bed centre) is a plain G1 with no macro side effect,
    # so a re-capture after it would be harmless here, not dangerous - but Q2's
    # qidi_pa_measure.py fixed the same PATTERN where it IS dangerous, and
    # carrying two different rules for when to re-capture invites exactly that
    # mistake. One rule: capture once, before anything moves.
